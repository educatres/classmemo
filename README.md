...
資料庫規則要求已通過 Firebase Authentication 的使用者才能讀寫。白板採匿名協作設計：任何持有白板連結的使用者都能查看；未凍結時，學生可以修改便條貼。凍結、刪除、JSON 下載與 JSON 匯入則由該白板已授權的老師裝置限制，因此請勿將老師密鑰分享給學生。匯入只會回復便條貼，不會覆寫本白板的老師權限、凍結狀態或到期時間。
