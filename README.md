...
- 老師登入後可凍結學生編輯、清除或刪除本白板，並下載本白板的 JSON 資料。
...
資料庫規則要求已通過 Firebase Authentication 的使用者才能讀寫。白板採匿名協作設計：任何持有白板連結的使用者都能查看；未凍結時，學生可以修改便條貼。凍結、刪除與 JSON 下載則由該白板已授權的老師裝置限制，因此請勿將老師密鑰分享給學生。
